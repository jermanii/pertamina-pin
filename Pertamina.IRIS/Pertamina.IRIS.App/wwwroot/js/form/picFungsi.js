$(function () {
    ...
    dt = $("#dataTable").DataTable({
        ...
        columns: [
            ...
            { data: 'NameFungsi', "name": "NameFungsi" },
            { data: 'PicName', "name": "PicName" },
        ...
        columnDefs: [
            {
                ...
                render: function (data, type, row) {
                    ...
                    return "" +
                        ...
                        "<a class='menu-link px-3' data-kt-docs-table-filter='edit_row' onclick='updateRedirect(\"" + data.Id + "\",\"" + data.FungsiId + "\")'>" +
                        "Edit" +
                        "</a>" +
                        "</div>" +
                        "<div class='menu-item px-3'>" +
                        ...
                        if (status == 'Valid') {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    var model = {};
                                    model.FungsiId = $("#ddlFungsiId").val();
                                    model.PicName = $("#picNameInput").val();
                                    model.Phone = $("#phoneInput").val();
                                    model.Email = $("#emailInput").val();
...
function updateRedirect(guid, fungsiId) {
    ...
    $.get(urlPartial).done(function (response) {
                    ...
                    if (status == 'Valid') {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                var model = {};
                                model.Id = $("#Id").val();
                                model.FungsiId = $("#ddlFungsiIdUpdate").val();
                                model.PicName = $("#picNameInput").val();
                                model.Phone = $("#phoneInput").val();
                                model.Email = $("#emailInput").val();
        ...
        ajaxPreselecting(url.getDdlFungsi + "?guid=" + fungsiId, $('#ddlFungsiIdUpdate'));
    });
};
