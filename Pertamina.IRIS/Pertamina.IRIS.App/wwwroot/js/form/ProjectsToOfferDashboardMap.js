...
worldSeries.mapPolygons.template.events.on("click", (ev) => {
    ...
    Promise.all([
        ...
        $.ajax({
            type: 'GET',
            url: param.urlProvincesMap,
            success: function (data) {
                var ProvinceAcronymsOfArray = []
                var DataHover = []
                data.forEach(function (datas) {
                    var provinceAcronyms = datas.ProvinceAcronyms;
                    ProvinceAcronymsOfArray.push(provinceAcronyms);
                });

                let countryId = ProvinceAcronymsOfArray;
                let color = am5.color(0xB6CAD8);
                geodata.features.forEach(function (Geodata) {
                    if (countryId.includes(Geodata.id)) {
                        countrySeries.mapPolygons.each(function (polygon) {
                            if (polygon.dataItem.get("id") === Geodata.id) {
...
countrySeries.mapPolygons.template.events.on("click", function (ev) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var HshOfArray = [];
            data.forEach(function (datas) {
                if (datas.ProvinceAcronyms == dataContext.id) {
                    var provinceAcronyms = datas.ProvinceAcronyms;
                    ProvinceAcronymsOfArray.push(provinceAcronyms);

                    if (datas.NamaProyekss.length > 0) {
                        var hsHHtml = "<ul>";
                        var provinsi = "<span>"
                        datas.NamaProyekss.forEach(function (hsH, index) {
                            hsHHtml += '<li>' + hsH.NamaProyeks;
                            if (index === 0) {
                                provinsi += hsH.LokasiProyek
                            }

                        })
                        provinsi += "<span>"
                        hsHHtml += "<ul>";
                        if (ProvinceAcronymsOfArray != undefined) {
                            ev.target.set("tooltipHTML", "<div style='color:black'>" + provinsi + "<strong> :</strong><br>" + hsHHtml + "</div>");
                            ev.target.showTooltip();
                        }

                    } else {
                        ev.target.set("tooltipHTML", "");
...
countrySeries.mapPolygons.template.events.on("pointerover", function (ev) {
    ...
    countrySeries.set("tooltip", am5.Tooltip.new(root, {
...
for (var id in am5geodata_data_countries2) {
    if (am5geodata_data_countries2.hasOwnProperty(id)) {
        ...
        if (country.maps.length && id == "ID") {
            data.push({
                id: id,
                map: country.maps[0],
                polygonSettings: {
                    fill: am5.color(0xB6CAD8),
                }
            });
        }
        if (country.maps.length && id != "ID") {
