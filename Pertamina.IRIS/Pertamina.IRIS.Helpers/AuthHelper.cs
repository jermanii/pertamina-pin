...
namespace Pertamina.IRIS.Helpers
{
    public static class AuthHelper
    {
        public static HttpClient ClientIdamanBearear(IHttpContextAccessor httpContextAccessor, AppSettingsBaseModel appSettingsViewModel)
        {
        ...
        public static string GetTokenIdaman(HttpClient client, IHttpContextAccessor httpContextAccessor, AppSettingsBaseModel appSettingsViewModel)
        {
            string token;

            if (!CookieHelper.IsExistCookie(SecurityHelper.ToBase64Encode(appSettingsViewModel.ApplicationCookiesName + ".Idaman.Token"),
           httpContextAccessor))
            {
        ...
        private static string GenerateTokenIdaman(HttpClient client, IHttpContextAccessor httpContextAccessor, AppSettingsBaseModel appSettingsViewModel)
        {
            if (client != null)
            {
